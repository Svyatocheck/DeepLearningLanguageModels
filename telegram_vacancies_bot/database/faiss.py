...
def faiss_search_rubert(file_name: str, index, dataframe: pd.DataFrame, top_k: int = 4):
...
def faiss_search_e5(file_name: str, index, dataframe: pd.DataFrame, top_k: int = 4):
    query = get_embed_e5(file_name)
    D, I = index.search(query.detach().numpy(), top_k)
    top_k_vacancy = dataframe.iloc[I[0]]
    return top_k_vacancy
